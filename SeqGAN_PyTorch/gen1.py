...
class Generator(nn.Module):
    ...
    def pretrain_forward(self, x, hidden):
        """
        Args:
            x: (batch_size, seq_len), sequence of tokens generated by generator
            class_label: (batch_size, ), class label for the sequences
        ...
        x = x.to(self.emb.weight.device)
        hidden = tuple(h.to(self.emb.weight.device) for h in hidden)
        pretrain_emb = self.pretrain_emb(x) # (batch_size, seq_len, emb_dim)

        output, hidden = self.lstm(pretrain_emb, hidden)
        pred = self.lin(output.contiguous().view(-1, self.hidden_dim))
    ...
    def generate(self, class_label):
        ...
        for i in range(self.sequence_length):
            logits, hidden = self.pretrain_forward(x, hidden)
    ...
    def pretrain_step(self, x):
        ...
        # Forward pass
        logits, _ = self.pretrain_forward(x, hidden)
