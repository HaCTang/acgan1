...
class Rollout(nn.Module):
    ...
    def forward(self, x, class_label, given_num):
        ...
        emb = self.g_embeddings(x).view(self.sequence_length, self.batch_size, self.emb_dim)
        ...
        self.processed_x = torch.cat([emb, class_emb], dim=-1)  # (seq_len, batch_size, emb_dim * 2)
        ...
        for i in range(given_num, self.sequence_length):
            x_t = self.processed_x[i] if i == given_num else self.g_embeddings(next_token)
            h_tm1 = self.g_recurrent_unit(x_t, h_tm1)
    ...
    def update_params(self):
        ...
        with torch.no_grad():
            self.g_embeddings = self.lstm.emb.clone()
